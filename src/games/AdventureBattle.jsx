    ...
    if ((screen === 'WIN' || screen === 'LOSE') && !isScoreSaved) {
        const autoSave = async () => {
            ...
            try {
                ...
                console.error(e);
                setSaveStatus("❌ Network Error. Results not saved.");
            }
        };

        autoSave();
    }
  }, [screen, isScoreSaved, gameId, score]);
          ...
          <div style={gameBoxStyle}>
            ...
            <div style={overlayStyle}>
              ...
              <div style={{
                  ...
                  borderRadius: '8px', 
                  marginBottom: '30px',
                  border: isScoreSaved ? '1px solid #48bb78' : '1px dashed #aaa'
              }}>
                  <p style={{color: isScoreSaved ? '#48bb78' : '#fbd38d', fontSize: '1.2rem', margin: 0, fontWeight: 'bold'}}>
                      {saveStatus}
                  </p>
              </div>
              
              <div style={{display: 'flex', gap: '15px'}}>
                  <button onClick={() => { playClick(); navigate('/student-menu'); }} style={{...mainBtnStyle, background: '#3182ce', fontSize: '1.2rem', padding: '15px 40px'}}>
                      🚪 EXIT TO MENU
                  </button>
              </div>
            </div>
