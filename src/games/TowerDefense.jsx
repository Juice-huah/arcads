...
// 🟢 SOUND IMPORTS
import menuBGM from '../defensetower/sounds/main_menu.mp3';
import gameBGM from '../defensetower/sounds/ingame.mp3';
import bossBGM from '../defensetower/sounds/boss_wave.mp3';
import victorySFX from '../defensetower/sounds/victory.mp3';
import clickSFX from '../defensetower/sounds/click.mp3';
...
function MainMenu({ onStart, navigate, disabled }) {
    ...
    <div style={styles.menu}>
      <div style={{animation:"slideUp 0.7s ease both", textAlign:"center", position:"relative", zIndex:1}}>
        ...
        <h1 style={{
          ...
          color: "#8b4513", textShadow: "2px 2px 0px #d2a679, 4px 4px 8px rgba(0,0,0,0.2)", margin: "10px 0"
      ...
      <div style={{display:"flex",flexDirection:"column",gap:14,alignItems:"center",width:"100%",maxWidth:340,animation:"slideUp 0.7s 0.2s ease both",position:"relative",zIndex:1}}>
        <button
          style={{
            ...
            border:"2px solid #e65100", borderRadius:8, cursor: disabled ? "not-allowed" : "pointer", textTransform:"uppercase",
            background: disabled ? "#78716c" : "linear-gradient(135deg, #f57c00, #e65100)", color:"#fff",
            boxShadow: hoverPlay && !disabled ? "0 6px 20px rgba(230, 81, 0, 0.5)" : "0 4px 12px rgba(0,0,0, 0.3)",
            transform: hoverPlay && !disabled ? "scale(1.02) translateY(-2px)" : "none", transition:"all 0.2s ease",
            opacity: disabled ? 0.6 : 1
          }}
          onMouseEnter={()=>setHoverPlay(true)} onMouseLeave={()=>setHoverPlay(false)} onClick={disabled ? undefined : onStart}
          disabled={disabled}
        >
          {disabled ? "Loading/Missing Content" : "🏰 Begin the Siege"}
        </button>
        <button
          style={{
...
export default function TowerDefense() {
  ...
  useEffect(() => {
    ...
    bgmMenuRef.current.volume = 0.5;

    bgmGameRef.current = new Audio(gameBGM);
    bgmGameRef.current.loop = true;
    bgmGameRef.current.volume = 0.35;

    bgmBossRef.current = new Audio(bossBGM);
    bgmBossRef.current.loop = true;
    bgmBossRef.current.volume = 0.45;

    victoryAudioRef.current = new Audio(victorySFX);
    victoryAudioRef.current.volume = 0.7;
  ...
  useEffect(() => {
    ...
    if (screen === 'menu' || screen === 'mapselect') {
        ...
        bgmBossRef.current.currentTime = 0;
    ...
    const fetchQuestions = async () => {
      try {
        ...
        if (data && data.length > 0) {
          ...
          const activeWaves = order.filter(diff => mapped.some(q => q.difficulty === diff));
          setWaveSequence(activeWaves);
        }
      } catch (err) {
  ...
  const isContentValid = questions.length > 0 && waveSequence.length > 0;
            ...
            <>
                ...
                {!isContentValid && !loading && (
                    <div style={{ position: 'absolute', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <div style={{ background: '#1e293b', border: '2px solid #ef4444', padding: '30px', borderRadius: '15px', textAlign: 'center', maxWidth: '400px' }}>
                            <div style={{ fontSize: '3rem', marginBottom: '10px' }}>⚠️</div>
                            <h2 style={{ fontFamily: FONT, color: '#fca5a5', margin: '0 0 10px 0' }}>CONTENT INCOMPLETE</h2>
                            <p style={{ color: '#cbd5e1', fontSize: '1rem', marginBottom: '20px' }}>Your teacher has not provided enough questions to start this game. Please check back later.</p>
                            <button onClick={() => navigate('/student-menu')} style={{ background: '#ef4444', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '8px', cursor: 'pointer', fontFamily: FONT, fontWeight: 'bold' }}>Return to Arcade</button>
                        </div>
                    </div>
                )}
                <MainMenu onStart={()=>setScreen("mapselect")} navigate={navigate} disabled={!isContentValid || loading} />
            </>
        )}
