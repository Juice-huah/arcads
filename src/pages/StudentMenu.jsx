...
const StudentMenu = () => {
  ...
  // 🟢 FIXED: Bulletproof Game Router Function
  const getGameRoute = (gameType, gameId) => {
      // Force it to lowercase to prevent capitalization errors
  ...
  return (
    <div className="teacher-dashboard">
      ...
      <div className="sidebar">
        ...
        <button className={`sidebar-btn ${activeTab === 'leaderboard' ? 'active' : ''}`} onClick={() => {setActiveTab('leaderboard'); setSelectedClass(null); setLeaderboardView('list');}}>Leaderboard</button>
      </div>
            ...
            <>
                            ...
                            {Object.keys(groupedGames).length === 0 ? (
                                ...
                                Object.keys(groupedGames).map((className) => (
                                    <div key={className} className="class-card" onClick={() => setSelectedClass(className)} style={{ overflow: 'hidden', padding: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: '150px' }}>
                                        <h3 style={{ color: '#fff', whiteSpace: 'normal', wordBreak: 'break-word', overflowWrap: 'anywhere', lineHeight: '1.4', textAlign: 'center', margin: '0 0 10px 0', width: '100%' }}>{className}</h3>
                        ...
                        <div className="classes-grid">
                            {groupedGames[selectedClass].map((game) => (
                                <div key={game.game_id} className="class-card" style={{ borderColor: '#0ac8f0', overflow: 'hidden', padding: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: '150px' }}>
                                    <h3 style={{ color: '#0ac8f0', whiteSpace: 'normal', wordBreak: 'break-word', overflowWrap: 'anywhere', lineHeight: '1.4', textAlign: 'center', margin: '0 0 10px 0', width: '100%' }}>
                                      {game.game_type.replace('_', ' ').toUpperCase()}
                                    </h3>
                                    <p style={{fontSize: '0.8rem', color: '#fff'}}>Assigned by Prof. {game.teacher_surname}</p>
                                    
                                    {/* 🟢 FIXED: Safely resolves the exact route for Hamsterball! */}
                                    <Link to={getGameRoute(game.game_type, game.game_id)} style={{width: '100%'}}>
                                        <button className="btn btn-primary" style={{width: '100%', marginTop: '15px'}}>START GAME</button>
                                    </Link>
                ...
                {activeTab === 'leaderboard' && leaderboardView === 'list' && (
                    <>
                        ...
                        <div className="classes-grid">
                            {Object.keys(groupedGames).map((className) => (
                                <div key={className} className="class-card" style={{ border: '1px solid #ffd700', overflow: 'hidden', padding: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: '150px' }} onClick={() => { setSelectedClass(className); setLeaderboardView('games'); }}>
                                    <h3 style={{ color: '#ffd700', whiteSpace: 'normal', wordBreak: 'break-word', overflowWrap: 'anywhere', lineHeight: '1.4', textAlign: 'center', margin: '0 0 10px 0', width: '100%' }}>{className}</h3>
                ...
                {activeTab === 'leaderboard' && leaderboardView === 'games' && selectedClass && (
                    <>
                        ...
                        <div className="classes-grid">
                            {groupedGames[selectedClass].map((game) => (
                                <div key={game.game_id} className="class-card" onClick={() => fetchLeaderboard(game)} style={{ overflow: 'hidden', padding: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: '150px' }}>
                                    <h3 style={{ color: '#0ac8f0', whiteSpace: 'normal', wordBreak: 'break-word', overflowWrap: 'anywhere', lineHeight: '1.4', textAlign: 'center', margin: '0 0 10px 0', width: '100%' }}>
                                      {game.game_type.replace('_', ' ').toUpperCase()}
                                    </h3>
